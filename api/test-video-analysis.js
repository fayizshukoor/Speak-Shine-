 ...
 * Run this to verify the web video upload system is working
 */
...
async function testVideoAnalysis() {
  console.log("🧪 Testing Video Analysis System...\n");
  ...
  try {
    ...
    // Test 1: Check TTL index exists
    console.log("\n📋 Test 1: Checking TTL index...");
    const indexes = await VideoReport.collection.getIndexes();
    const ttlIndex = indexes.find(idx => idx.expiresAt && idx.expireAfterSeconds === 0);
    if (ttlIndex) {
      console.log("✅ TTL index found - reports will auto-delete after 12 hours");
    } else {
      console.log("❌ TTL index not found - reports may not auto-delete");
    }
    ...
    // Test 4: Update report to completed
    console.log("\n🔄 Test 4: Updating report status...");
    await VideoReport.findByIdAndUpdate(testReport._id, {
      status: "completed",
      analysis: {
        fluency: 8,
        grammar: 7,
    ...
    console.log("✅ Report updated to completed status");

    // Test 5: Retrieve report
    console.log("\n📖 Test 5: Retrieving report...");
    ...
    // Test 7: List user reports
    console.log("\n📋 Test 7: Listing user reports...");
    ...
    // Test 8: Check if video processing would work (without actual video)
    console.log("\n🎬 Test 8: Testing video processor (dry run)...");
    try {
      // This will fail because we don't have a real video file, but it tests the import
...
📝 Great job! Your speaking has improved significantly.`;
    ...
    console.log("   ✅ TTL index configured for auto-deletion");
    ...
    console.log("   ✅ Expiration system working");
